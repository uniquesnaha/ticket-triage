...
async def require_api_key(x_api_key: str | None = Security(api_key_header)) -> str | None:
    """Validate the X-API-Key header (timing-safe) when TRIAGE_API_KEY is configured."""
    configured = get_settings().triage_api_key
    if configured is None:
        return None  # open deployment
